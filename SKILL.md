...
19. **分镜五字段固定结构**：每个时间段必须拆为景别/运镜/画面/台词/特效五个固定字段，顺序不可调换；画面字段只写画面内容（人物动作/表情/环境变化），不写台词内容；台词字段独立，格式为`台词起止时间丨角色名丨"台词内容"（口型对准/画外音丨台词分类）`；特效字段必须自包含（等级+类型+颜色分层/动态形态/光影影响/粒子细节/时间控制）。
