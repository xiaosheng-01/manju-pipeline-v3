...
| 🟡阶段 | `04-production/ch19-storyboard-cases.md` | **为什么必须索引：分镜画面字段写法有大量高频系统性错误（人物外观名词罗列、表情只有肯定没有否定、动作触发时间模糊、画外音写"说"、反应镜头没有触发词等），这些错误每个分镜都会遇到，不是偶尔才遇到，所以必须每次都索引对照检查，不能按需。** 定义了正确vs错误分镜案例库，覆盖8类场景20+个常见错误点 | **索引后对当前步骤的影响：** 每个子分镜写完后，必须对照ch19第19.9节的10项检查清单逐项检查（人物外观动词/表情三层结构/程度词否定边界/台词触发动作位置/画外音写法/动作触发字词绑定/运镜节点穿插/群像空间关系/反应镜头三段/情绪渐变因果），检查完才允许输出。索引的好处：把这几轮试错出来的正确写法固化成案例，防止AI每次都犯同样的错误，分镜质量稳定 | 分镜画面字段写法不规范，AI生成质量差，每次都要人工纠正 |
