...
5. **最小强制加载集**：
   ...
   - 非仙侠题材（武侠/都市/校园/农村/悬疑等）：`02-style/non-xianxia-styles.md` + `06-library/non-xianxia-cases.md`（仙侠题材默认不加载）。
...
| 🟡阶段 | `04-production/ch19-storyboard-cases.md` | **分镜四字段融合总控，P0强制执行规则，不是参考案例。** 包含四字段结构定义、画面字段9子维度导航、融合协调规则、具体度校验、表情输出格式、合格范例。ch19第19.0.1节有"规则执行时机绑定表"，明确20条规则分别在分镜流程的哪个步骤强制执行。到了对应步骤，必须立即执行对应规则，禁止跳过、绕过、只读不做。每条规则违反即退回重写 | **索引后对当前步骤的影响：** 分镜流程步骤8（写画面字段）时，写到哪个字段就强制执行ch19绑定表对应的规则；步骤15（评分）时核对检查清单；步骤16（输出前）强制过检查清单，全部通过才允许输出。**读了ch19但不执行 = 未执行 = 分镜作废。** | 违反ch19任一条规则，或只读不做，该子分镜退回重写 |
...
每镜强制流程卡 0-16 步、分镜固定结构（首帧/尾帧/四字段顺序/第一个镜头写法），详见 
eferences/00-core/ch01-control.md 第 1.5 节。写分镜时按 ch01.5 的流程卡逐步执行，输出前对照 ch01.5 的检查清单逐项核对。
