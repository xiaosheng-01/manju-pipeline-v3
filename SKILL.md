...
17. **大纲使用边界**：作者已有小说正文且持续更新时，大纲仅作章节走向、前后衔接、角色弧线方向的参考，是"参考思路"不是"强制契约"；大纲不作为任务输出必备项目，分镜内容只由该集正文决定；禁止以大纲为基础创作正文里没有的剧情、台词、细节，禁止从大纲编造角色外貌/服装/场景描述（只能从正文提取）；禁止强制对齐大纲的伏笔回收时间点（大纲写第10集回收，作者实际可能第15-16集才回收，以正文实际位置为准）；大纲不具备完全认知，可能滞后或被作者修改；做某集时该集内容以该集正文为唯一依据，正文与大纲不一致时以正文为准。
