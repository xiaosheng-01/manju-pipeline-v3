...
| 🟡阶段 | `04-production/ch19-storyboard-cases.md` | **P0强制执行规则，不是参考案例。** 分镜画面字段写法有大量高频系统性错误（人物外观名词罗列、表情只有肯定没有否定、动作触发时间模糊、画外音写"说"、反应镜头没有触发词等），这些错误每个分镜都会遇到，不是偶尔才遇到，所以必须每次都索引强制执行，不能按需。覆盖8类场景20+个常见错误点，每条规则违反即退回重写 | **索引后对当前步骤的影响：** 每个子分镜写完后，必须对照ch19第19.9节12项检查清单逐项检查，12项全部通过才允许输出。任一项不通过，该子分镜退回重写。索引的好处：把这几轮试错出来的正确写法固化成P0铁律，防止AI每次都犯同样的错误，分镜质量稳定 | 违反ch19任一条规则，该子分镜退回重写，分镜画面字段写法不规范，AI生成质量差 |
