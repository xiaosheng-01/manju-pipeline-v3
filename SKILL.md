...
15. 内部评分（≥90）
   → **评分时必须核对ch19第19.9节12项检查清单**，任一项未执行扣分，严重者退回重写
16. 通过后输出
   → **输出前强制过ch19第19.9节12项检查清单**，12项全部通过才允许输出。未检查就输出，该镜作废。**绝对禁止只读ch19不执行、跳过检查、绕过规则。**
```
