...
分镜固定结构：首帧 0.5 秒远景定场（自然微动、不静止）→ 核心内容 → 尾帧 0.5 秒远景定场（尾韵微动，为下一镜首帧提供衔接）。首尾帧禁放台词（先声后画/声画分离除外）。每个时间段六字段固定顺序：**景别 → 运镜 → 画面 → 台词 → 音效 → 字幕**。特效描述直接融入画面字段里，不单独列特效字段。第一个镜头首帧写"本镜为第一个镜头，无上一镜承接"，第二个镜头开始首帧写"承接上一镜尾帧"。画面描述带编号加全名（R-01凌渊），同一子分镜内第二次提及可省略编号，跨子分镜再出现时重新写全编号加全名。
