...
```text
1. 台词八道关（语境关→动作关→逻辑关→精华关→替代关→情绪语气语速关→时长吞没关→核心意思锁定关，每关必须强制执行，禁止打勾跳过；精华关必须强制执行使用高级压缩技巧，替代关必须强制执行判断能否用画面替代，压缩后必须回读三问）→ 台词时长预检（字数→红线=字数÷4.5→目标=字数÷3.5+节奏缓冲+情绪缓冲→核心可用=总时长-1秒→是否放得下）→ 多句合计预检（一镜≥2句必须做合计，单句过但合计不过视为未过）；三步全过才允许写分镜
