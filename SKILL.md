...
20. **分镜五字段固定结构**：每个时间段必须强制拆为景别/运镜/画面/音效/字幕，5个固定字段，顺序不可调换；台词内容和特效描述都直接融入画面字段里，不单独列台词字段和特效字段；画面字段必须自包含（人物动作+表情+环境变化+台词内容（如有）+特效描述（如有））。
...
| 🟡阶段 | `04-production/ch12-storyboard-template.md` | 定义了分镜模板和标准格式，是分镜输出的直接模板 | 知道分镜的五字段结构、标准格式、首尾帧规则 | 分镜格式混乱、字段缺失、输出不规范 |
...
8. 逐段填负载（景别/运镜/画面/音效/字幕五字段，台词和特效融入画面里）
...
15. 内部评分（≥90）
   → **评分时必须核对ch19第19.16节17项检查清单**，任一项未执行扣分，严重者退回重写
16. 通过后输出
   → **输出前强制过ch19第19.16节17项检查清单**，17项全部通过才允许输出。未检查就输出，该镜作废。**绝对禁止只读ch19不执行、跳过检查、绕过规则。**
```

分镜固定结构：首帧 0.5 秒远景定场（自然微动、不静止）→ 核心内容 → 尾帧 0.5 秒远景定场（尾韵微动，为下一镜首帧提供衔接）。首尾帧禁放台词（先声后画/声画分离除外）。每个时间段五字段固定顺序：**景别 → 运镜 → 画面 → 音效 → 字幕**。台词内容和特效描述都直接融入画面字段里，不单独列台词字段和特效字段。第一个镜头首帧写"本镜为第一个镜头，无上一镜承接"，第二个镜头开始首帧写"承接上一镜尾帧"。画面描述带编号加全名（R-01凌渊），同一子分镜内第二次提及可省略编号，跨子分镜再出现时重新写全编号加全名。
